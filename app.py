...
_DEFAULTS = {
    ...
    "ch_lang": "Python", "ch_diff": "medium", "ch_topic": "arrays",
...
def _need_key() -> bool:
    """Returns True if the GROQ_API_KEY is missing (shows error in UI)."""
    try:
        from dotenv import load_dotenv
        import os as _os
        _base = _os.path.dirname(_os.path.abspath(__file__))
        load_dotenv(_os.path.join(_base, ".env"), override=False)
        load_dotenv(_os.path.join(_os.getcwd(), ".env"), override=False)
    except ImportError:
        pass
    ...
    if not key:
        st.markdown("""
        <div class="msg-err">
        🔑 <b>GROQ_API_KEY not configured.</b><br>
        1. Get your free key at <a href="https://console.groq.com" target="_blank" style="color:#EF4444;">console.groq.com</a><br>
        2. Create a <code>.env</code> file in the same folder as <code>app.py</code><br>
        3. Add this line: <code>GROQ_API_KEY=gsk_your_key_here</code><br>
        4. Restart the app with: <code>python -m streamlit run app.py</code>
        </div>
        """, unsafe_allow_html=True)
        return True
    return False
...
def page_dashboard():
            ...
            if not _need_key():
                if len(code_input.strip()) < 10:
                    ...
                    with st.spinner("🧠 Analyzing with llama-3.3-70b-versatile…"):
                        try:
                            result = ai_rv(code_input, language)
            ...
            if not _need_key():
                if len(tr_code.strip()) < 10: st.error("Please paste code to translate.")
                    ...
                    with st.spinner(f"🌐 Translating {src_lang} → {tgt_lang}…"):
                        try:
                            st.session_state.translate_result = translate_code(tr_code, src_lang, tgt_lang)
                        except Exception as e:
                            st.error(f"Error: {e}")
                    st.rerun()
            ...
            if not _need_key():
                if len(exp_code.strip()) < 10: st.error("Please paste some code.")
                    ...
                    with st.spinner("💡 Generating explanation…"):
                        try:
                            st.session_state.explain_result = explain_code(exp_code, exp_lang)
                        except Exception as e:
                            st.error(f"Error: {e}")
                    st.rerun()
    ...
    # ════════════════════════════════════════════════════════════════════════
        ...
        if gen_btn:
            if not _need_key():
                ...
                with st.spinner("🎲 Generating challenge…"):
                    try:
                        st.session_state.current_challenge = generate_challenge(ch_lang, ch_diff, ch_topic)
                        st.session_state.challenge_eval    = None
                        st.session_state.show_solution     = False
                        # Persist selections so challenge card can read them
                        st.session_state["ch_lang"] = ch_lang
                        st.session_state["ch_diff"] = ch_diff
                        st.session_state["ch_topic"] = ch_topic
                    except Exception as e:
                        st.error(f"Error: {e}")
                st.rerun()
        ...
        if st.session_state.current_challenge:
            ...
            # Use session state values which are always available
            _ch_diff  = st.session_state.get("ch_diff", "medium")
            _ch_lang  = st.session_state.get("ch_lang", "Python")
            diff_cls = {"easy":"diff-easy","medium":"diff-medium","hard":"diff-hard"}.get(_ch_diff, "diff-medium")
            st.markdown(f"""<div class="challenge-card">
              <div style="display:flex;align-items:center;gap:0.65rem;margin-bottom:0.5rem;flex-wrap:wrap;">
                ...
                <span class="diff-badge {diff_cls}">{_ch_diff}</span>
            ...
            ide_header(_ch_lang, "Your Solution")
            ...
            if st.session_state.get("show_solution") and ch.get("solution"):
                with st.expander("💡 Model Solution"):
                    st.code(ch["solution"], language=_ch_lang.lower() if _ch_lang in ["Python","JavaScript","Java","Go","Rust"] else "text")
            ...
            if eval_btn:
                if not _need_key():
                    ...
                    with st.spinner("🤖 Evaluating…"):
                        try:
                            ev = evaluate_challenge(user_sol, ch, _ch_lang)
                            st.session_state.challenge_eval = ev
                            save_challenge_attempt(user["id"], ch.get("title",""), _ch_lang,
        ...
        if dna_btn:
            if not dna_code.strip():
                ...
                with st.spinner("🧬 Sequencing your Code DNA…"):
                    result = analyze_code_dna(dna_code.strip(), dna_lang)
