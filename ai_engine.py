...
# ── Model ─────────────────────────────────────────────────────────────────────
# llama-3.3-70b-versatile: fastest on Groq, best JSON accuracy, no reasoning_effort needed
...
def _call(api_key: str, system: str, user: str, temperature: float = 0.2, max_tokens: int = 4096) -> str:
...
def review_code(api_key: str, code: str, language: str, context: str = "") -> dict:
    """
...
```"""
    ...
    text = _call(api_key, system, user, temperature=0.15, max_tokens=4096)
    try:
        ...
        return {
            "summary": f"⚠️ Could not parse review. Raw snippet: {text[:200]}",
...
def rewrite_code(api_key: str, code: str, language: str) -> dict:
    """
    ...
    system = (
        ...
        "You write production-grade, idiomatic {language}. "
...
```"""
    ...
    text = _call(api_key, system, user, temperature=0.1, max_tokens=4096)
    try:
        data = _extract_json(text)
...
def explain_code(api_key: str, code: str, language: str) -> str:
...
{code}
```"""
    return _call(api_key, system, user, temperature=0.3, max_tokens=2048)


def translate_code(api_key: str, code: str, source_lang: str, target_lang: str) -> dict:
...
```"""
    ...
    text = _call(api_key, system, user, temperature=0.2, max_tokens=4096)
    try:
        data = _extract_json(text)
        data.setdefault("translated_code", code)
        data.setdefault("notes", "Translation complete.")
        data.setdefault("idiom_changes", [])
        data.setdefault("warnings", "")
        return data
    except Exception:
        return {
            "translated_code": code,
            "notes": "Translation response could not be parsed.",
            "idiom_changes": [],
            "warnings": text[:400],
        }


def generate_challenge(api_key: str, language: str, difficulty: str, topic: str) -> dict:
...
{{
  ...
  "tags": ["tag1", "tag2"]
    ...
    text = _call(api_key, system, user, temperature=0.6, max_tokens=2048)
    try:
        data = _extract_json(text)
...
def analyze_code_dna(api_key: str, code: str, language: str) -> dict:
    """
...
```"""
    ...
    text = _call(api_key, system, user, temperature=0.2, max_tokens=3500)
    try:
        data = _extract_json(text)
        ...
        return {
            "archetype": "The Pragmatist", "archetype_desc": "Could not parse DNA.",
...
def interview_ask(api_key: str, code: str, language: str, history: list, user_answer: str = "") -> dict:
    """
    ...
    # Extract previous scores from history metadata if stored
    prev_scores = [m.get("score", 0) for m in history if m.get("role") == "score_record"]
...
{{
  ...
  "improvements": []
        ...
        # Collect all previous answer scores from history
        score_entries = [m for m in history if m.get("role") == "score_record"]
        accumulated = sum(m.get("score", 0) for m in score_entries)
...
- Each answer should have been scored 0-100 based on relevance and quality
...
{{
  ...
  "improvements": []
    ...
    text = _call(api_key, system, user_prompt, temperature=0.2, max_tokens=1800)
    try:
        data = _extract_json(text)
        # Clamp answer_score strictly
        raw_score = int(data.get("answer_score", 0))
        data["answer_score"] = max(0, min(100, raw_score))
        # Clamp cumulative_score
        cum = int(data.get("cumulative_score", 0))
        data["cumulative_score"] = max(0, min(100, cum))
        ...
        return {
            ...
            "evaluation": "", "final_feedback": "", "strengths": [], "improvements": [],
        }


def time_travel_code(api_key: str, code: str, language: str, era: str) -> dict:
    """
    ...
    system = (
        f"You are a programming historian and code archaeologist. "
        f"You deeply understand how coding styles, idioms, and tools evolved across decades. "
        f"Respond with a single valid JSON object only — no markdown, no prose outside JSON."
    )
...
```"""
    ...
    text = _call(api_key, system, user, temperature=0.3, max_tokens=4096)
    try:
        data = _extract_json(text)
        ...
        return {
            ...
            "era_code": code,
            "era_summary": "Could not parse time travel result.",
            "changes": [],
            "complexity_note": "",
...
def evaluate_challenge(api_key: str, user_code: str, challenge: dict, language: str) -> dict:
...
{{
  ...
  "test_results": [
    ...
    text = _call(api_key, system, user, temperature=0.2, max_tokens=1024)
    try:
        data = _extract_json(text)
        ...
        return {
            ...
            "feedback": text[:400], "improvements": [],
